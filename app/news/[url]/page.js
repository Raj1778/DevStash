    ...
    const fetchArticle = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Check if response was cached (you can add a header to indicate this)
          ...
          <div className="text-lg leading-relaxed text-zinc-300 space-y-6">
            ...
            {article.content ? (
              <div className="space-y-6">
                {article.content.split('\n\n').map((paragraph, index) => {
                  ...
                  if (paragraph.length < 100 && paragraph.endsWith(':')) {
                    return (
                      <h2 key={index} className="text-2xl font-semibold text-white border-b border-zinc-700 pb-2">
                        {paragraph}
                      </h2>
                    );
                  }
              ...
              <p className="text-base leading-relaxed text-zinc-300">
                {article.description || 'Content not available'}
              </p>
            )}
          </div>
        </article>
